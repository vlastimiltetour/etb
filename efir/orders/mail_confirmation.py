...
import os
import django
from smtplib import SMTPDataError, SMTPException, SMTPRecipientsRefused, SMTPSenderRefused
from django.core.mail import EmailMultiAlternatives
from django.http import HttpResponse
from django.template.loader import render_to_string
from .models import Order
...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'efir.settings.base')  # Replace 'myproject' with the name of your Django project
django.setup()
...
def customer_order_email_confirmation(order_id):
    try:
        logger.info("Attempting to send customer order confirmation email for order ID %s", order_id)

        order = Order.objects.get(id=order_id)
        pdf = render_to_string("orders/invoice_pdf.html", {"order": order})
        out = BytesIO()
        stylesheet_url = "https://etb.fra1.cdn.digitaloceanspaces.com/etb/css/styles.css"
        ...
        msg.send()
        logger.info("Customer order confirmation email sent successfully for order ID %s", order_id)
        return True

    except (SMTPDataError, SMTPException, SMTPRecipientsRefused, SMTPSenderRefused) as e:
        ...
        logger.info("Attempting to send customer order confirmation email for order ID without PDF %s", order_id)
        ...
        msg.send()
        logger.info("Customer order confirmation email sent successfully wihtout PDF for order ID %s", order_id)
...
def certificate_order_email_confirmation(order_id):
    try:
        ...
        logger.error("An unexpected error occurred: %s", e)
        logger.info("Attempting to send certificate order confirmation email for order ID without PDF %s", order_id)
        order = Order.objects.get(id=order_id)
        html_content = render_to_string(
            "orders/certificate_confirmation.html", {"order": order}
        )
